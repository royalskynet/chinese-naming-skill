...
| A4 | 安裝進索引 | `npx skills add` 成功 + lock.json 記錄 + 新 session 可見 | **PASS** | `npx skills add chinese-naming --yes` 完成，`skills-lock.json` 記錄正常，`hermes skills list` 顯示 `chinese-naming  local  enabled` |
| A5 | 發布 | GitHub public repo 存在、MIT、README/NOTICE 完整 | **PASS** | `gh repo create royalskynet/chinese-naming-skill --public` 成功，git push done，repo：https://github.com/royalskynet/chinese-naming-skill |